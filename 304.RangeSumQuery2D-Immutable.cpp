...
public:
    ...
    int sumRegion(int row1, int col1, int row2, int col2)
    {
        ...
        if (!row1)
        {
            if (!col1) return dp[row2][col2];
            return dp[row2][col2] - dp[row2][col1-1];
        }
        else if (!col1)
        {
            if (!row1) return dp[row2][col2];
...
int main()
{
    ...
    NumMatrix numMatrix(matrix);
    std::cout << numMatrix.sumRegion(0, 1, 2, 3) << endl;
    std::cout << numMatrix.sumRegion(1, 2, 3, 4) << endl;
    std::cout << numMatrix.sumRegion(2,1,4,3) << endl; // 8
    std::cout << numMatrix.sumRegion(1,1,2,2) << endl; // 11
    std::cout << numMatrix.sumRegion(1,2,2,4) << endl; // 12
    return 0;
}
