...
private:
    ...
    bool searchMatrix(const vector<vector<int>> &matrix, const int &target,
        ...
        if (minc == maxc) {
            return searchInColumn(matrix, target, minc, minr, maxr);
        }
        if (minr == maxr) {
            return searchInRow(matrix, target, minr, minc, maxc);
        }

        int left_bottom = matrix[maxr][minc];
        int right_top = matrix[minr][maxc];

        if (left_bottom == target || right_top == target) {
            return true;
        }
